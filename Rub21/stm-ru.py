...
#query = """insert into osm_changeset (rowid, user_id, osm_user, min_lon, min_lat, max_lon, max_lat, closed_at, num_changes) values (?, ?, ?, ?, ?, ?, ?, ?, ?)"""
#print(query)
#print("----------attrib")
...
def save(attrib):
    ...
    print(attrib.get('user'))
      #if(int(parse(attrib["created_at"]).strftime('%s'))>=1364774400): #from 04/20/2013
    #cur.execute(query,
     #   (attrib_id,
